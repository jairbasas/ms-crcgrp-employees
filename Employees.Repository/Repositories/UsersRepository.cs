...
namespace Employees.Repository.Repositories
{
    public class UsersRepository : IUsersRepository
    {
        ...
        public async Task<int> Register(Users users)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                try
                {
                    ...
                    parameters = GetParameters(users);
        ...
        public async Task<int> RegisterAsync(Users users, SqlConnection connection, SqlTransaction transaction) 
        {
            var parameters = new DynamicParameters();

            parameters = GetParameters(users);

            return await connection.ExecuteAsync(@"TRANSVERSAL.USERS_insert_update", parameters, transaction, commandType: CommandType.StoredProcedure);
        }

        #region

        private DynamicParameters GetParameters(Users users) 
        {
            var parameters = new DynamicParameters();

            parameters.Add("@poi_user_id", users.userId, DbType.Int32, ParameterDirection.Input);
